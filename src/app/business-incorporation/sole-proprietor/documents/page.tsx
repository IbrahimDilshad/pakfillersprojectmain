...
const requiredDocs = [
  { id: 'cnic', title: { en: 'Color Copy of CNIC', ur: 'شناختی کارڈ کی رنگین کاپی' } },
  { id: 'ownership', title: { en: 'Rental Agreement or Ownership Docs', ur: 'کرایہ نامہ یا ملکیت کے دستاویزات' } },
  { id: 'bill', title: { en: 'Latest Paid Utility Bill', ur: 'تازہ ترین ادا شدہ یوٹیلیٹی بل' } },
  { id: 'letterhead', title: { en: "Company's Letterhead", ur: 'کمپنی کا لیٹر ہیڈ' } },
];
    ...
    const service = services.find(s => t(s.title).toLowerCase().includes('sole proprietor'));
     if (service) {
        ...
        toast({
            ...
            description: t({ en: "Sole Proprietor Registration has been added to your cart.", ur: "واحد ملکیت رجسٹریشن آپ کی کارٹ میں شامل کر دی گئی ہے۔" })
        });
        router.push('/cart');
    } else {
         toast({
  ...
  return (
    <AppLayout pageTitle={t({ en: 'Upload Documents', ur: 'دستاویزات اپ لوڈ کریں' })}>
      <div className="max-w-4xl mx-auto">
        <Card>
          <CardHeader>
            <CardTitle>{t({ en: 'Upload Required Documents', ur: 'مطلوبہ دستاویزات اپ لوڈ کریں' })}</CardTitle>
            <CardDescription>
              {t({
