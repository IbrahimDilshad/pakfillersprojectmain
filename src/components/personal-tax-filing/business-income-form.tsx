...
export function BusinessIncomeForm({ form: passedForm, incomeType, basePath }: { form: any, incomeType: string, basePath: string }) {
    const { t } = useLanguage();
    const { control, watch } = useFormContext(); // Use useFormContext if this is a child of a FormProvider

    const withholdingOption = watch(`${basePath}.withholdingOption`);
    const hasOtherAdjustableTaxes = watch(`${basePath}.hasOtherAdjustableTaxes`);
    
    const { fields: otherTaxFields, append: appendOtherTax, remove: removeOtherTax } = useFieldArray({
        control: control,
        name: `${basePath}.otherAdjustableTaxes`
    });
        ...
        <div className="space-y-6">
            ...
            {withholdingOption === 'all' && (
                <div className="p-4 border rounded-lg space-y-4">
                     ...
                     <div className="grid md:grid-cols-3 gap-4">
                        <FormField control={control} name={`${basePath}.revenueWithTax.revenueAmount`} render={({ field }) => (
                         ...
                         <FormField control={control} name={`${basePath}.revenueWithTax.taxDeducted`} render={({ field }) => (
                        ...
                        <FormField control={control} name={`${basePath}.revenueWithTax.taxRate`} render={({ field }) => (
             ...
             {withholdingOption === 'none' && (
                <div className="p-4 border rounded-lg space-y-4">
                     ...
                     <FormField control={control} name={`${basePath}.revenueWithoutTax.revenueAmount`} render={({ field }) => (
                        <FormItem><FormLabel>{t({en: 'Enter Amount', ur: 'رقم درج کریں'})}</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormMessage /></FormItem>
                     )}/>
                </div>
            )}
            ...
            <div className="p-4 border rounded-lg space-y-4">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                    <FormField control={control} name={`${basePath}.directExpense`} render={({ field }) => (
                    ...
                    <FormField control={control} name={`${basePath}.indirectExpense`} render={({ field }) => (
            ...
            {/* Balance Sheet Section */}
             <div className="p-4 border rounded-lg space-y-4">
                ...
                <div className="grid md:grid-cols-3 gap-4">
                    <FormField control={control} name={`${basePath}.totalAssets`} render={({ field }) => (
                    ...
                    <FormField control={control} name={`${basePath}.totalLiabilities`} render={({ field }) => (
                    ...
                    <FormField control={control} name={`${basePath}.totalCapital`} render={({ field }) => (
            ...
            <div className="p-4 border rounded-lg space-y-4">
                ...
                <FormField control={control} name={`${basePath}.hasOtherAdjustableTaxes`} render={({ field }) => (
                    ...
                    <FormControl><RadioGroup onValueChange={field.onChange} defaultValue={field.value} className="flex gap-4"><FormItem className="flex items-center space-x-2"><FormControl><RadioGroupItem value="yes" id={`adj_yes_${incomeType}`} /></FormControl><FormLabel htmlFor={`adj_yes_${incomeType}`} className="font-normal">{t({en: 'Yes', ur: 'ہاں'})}</FormLabel></FormItem><FormItem className="flex items-center space-x-2"><FormControl><RadioGroupItem value="no" id={`adj_no_${incomeType}`} /></FormControl><FormLabel htmlFor={`adj_no_${incomeType}`} className="font-normal">{t({en: 'No', ur: 'نہیں'})}</FormLabel></FormItem></RadioGroup></FormControl>
                    <FormMessage />
                    </FormItem>
                )}/>
                    ...
                    <div className="space-y-4 pt-2 animate-in fade-in-50">
                        {otherTaxFields.map((field, index) => (
                             <div key={field.id} className="grid md:grid-cols-[1fr_auto_auto] gap-4 items-end">
                                <FormField control={control} name={`${basePath}.otherAdjustableTaxes.${index}.description`} render={({ field }) => (
                                ...
                                <FormField control={control} name={`${basePath}.otherAdjustableTaxes.${index}.taxDeducted`} render={({ field }) => (
                                    <FormItem><FormLabel>{t({en: 'Tax Deducted', ur: 'ٹیکس کٹوتی'})}</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormMessage /></FormItem>
                                )}/>
    ...
    return (
        <div className="mt-4 p-4 border rounded-md bg-muted/20 space-y-6">
            ...
            <FormField
                control={control}
                name={`${basePath}.withholdingOption`}
                render={({ field }) => (
                    <FormItem>
                    <FormControl>
